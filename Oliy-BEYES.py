...
df['Has_Oliy'] = df['Tag'].str.contains('Oliy').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Oliy'].values
y_valid = df[df['dataset'] == 'val']['Has_Oliy'].values
y_test = df[df['dataset'] == 'test']['Has_Oliy'].values
...
checkpoint_dir = "./241214-Oliy-youhua"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 1.2040824620714006e-06,
        'num_attention_heads': 64,
        'dropout_rate': 0.6771127690067165,
        'dense_units': 512,
        'message_steps': 32,
        'batch_size': 1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_364_val_loss_0.11954500.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Oliy-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Oliy-valid-result.csv")
        ...
        test_results_df['Oliy-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Oliy-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Oliy'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Oliy'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
