...
df['Has_Woody'] = df['Tag'].str.contains('Woody').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Woody'].values
y_valid = df[df['dataset'] == 'val']['Has_Woody'].values
y_test = df[df['dataset'] == 'test']['Has_Woody'].values
...
checkpoint_dir = "./241214-Woody-youhua2"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 4.275708428664875e-06,
        'num_attention_heads': 32,
        'dropout_rate':  0.5512561681608958,
        'dense_units': 256,
        'message_steps': 8,
        'batch_size': 1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_492_val_loss_0.21665232.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Woody-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Woody-valid-result.csv")
        ...
        test_results_df['Woody-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Woody-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Woody'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Woody'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
    ...
    specified_indices = [3 ,1865]  # 指定你想要的多个样本索引
