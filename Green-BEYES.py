...
df['Has_Green'] = df['Tag'].str.contains('Green').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Green'].values
y_valid = df[df['dataset'] == 'val']['Has_Green'].values
y_test = df[df['dataset'] == 'test']['Has_Green'].values
...
checkpoint_dir = "./241214-Green-youhua"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 1.609540971241006e-05,
        'num_attention_heads': 16,
        'dropout_rate':  0.15175381109938818,
        'dense_units': 64,
        'message_steps': 8,
        'batch_size': 1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_183_val_loss_0.15900709.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Green-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Green-valid-result.csv")
        ...
        test_results_df['Green-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Green-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Green'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Green'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
