...
df['Has_Offensive'] = df['Tag'].str.contains('Offensive').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Offensive'].values
y_valid = df[df['dataset'] == 'val']['Has_Offensive'].values
y_test = df[df['dataset'] == 'test']['Has_Offensive'].values
...
checkpoint_dir = "./241219-Offensive-youhua"

if train == 1:
    # 初始化全局变量
    best_validation_loss = np.inf
    checkpoint_dir = "./241214-Offensive-youhua2"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 2.6221787001634256e-06,
        'num_attention_heads': 8,
        'dropout_rate':  0.14750916981136405,
        'dense_units': 64,
        'message_steps': 4,
        'batch_size': 1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_202_val_loss_0.16880885.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Offensive-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Offensive-valid-result.csv")
        ...
        test_results_df['Offensive-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Offensive-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Offensive'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Offensive'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
    ...
    specified_indices = [5216, 6189, 6053, 6096]  # 指定你想要的多个样本索引
