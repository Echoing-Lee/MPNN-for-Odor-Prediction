...
df['Has_Fruity'] = df['Tag'].str.contains('Fruity').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Fruity'].values
y_valid = df[df['dataset'] == 'val']['Has_Fruity'].values
y_test = df[df['dataset'] == 'test']['Has_Fruity'].values
...
checkpoint_dir = "./241219-Fruity-youhua"

if train == 1:
    # 初始化全局变量
    best_validation_loss = np.inf
    checkpoint_dir = "./241214-Fruity-youhua2"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 1.9709038082484368e-05,
        'num_attention_heads': 16,
        'dropout_rate': 0.176701659348324,
        'dense_units': 128,
        'message_steps': 8,
        'batch_size':1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_100_val_loss_0.22220397.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Fruity-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Fruity-valid-result.csv")
        ...
        test_results_df['Fruity-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Fruity-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Fruity'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Fruity'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
    ...
    specified_indices = [506, 1434,2442, 6071, 626]  # 指定你想要的多个样本索引
