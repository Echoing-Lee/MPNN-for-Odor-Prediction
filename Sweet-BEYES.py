...
df['Has_Sweet'] = df['Tag'].str.contains('Sweet').astype(int)
...
y_train = df[df['dataset'] == 'train']['Has_Sweet'].values
y_valid = df[df['dataset'] == 'val']['Has_Sweet'].values
y_test = df[df['dataset'] == 'test']['Has_Sweet'].values
...
checkpoint_dir = "./241219-Sweet-youhua"

if train == 1:
    # 初始化全局变量
    best_validation_loss = np.inf
    checkpoint_dir = "./241214-Sweet-youhua2"
...
if testa == 1:
    ...
    best_params = {
        'learning_rate': 1.3344043387485946e-05,
        'num_attention_heads': 8,
        'dropout_rate':  0.22420254784662796,
        'dense_units': 512,
        'message_steps': 8,
        'batch_size': 1
    }
    ...
    # 加载最佳权重文件
    checkpoint_filepath = os.path.join(checkpoint_dir, "model_epoch_36_val_loss_0.19073069.h5")  # 替换为你的最佳权重文件
    ...
    with open(output_file_path, 'w') as f, redirect_stdout(f):
        ...
        valid_results_df['Sweet-ypred'] = y_pred  # 保存预测值（非1/0）
        ...
        valid_results_csv_filepath = os.path.join(checkpoint_dir, "Sweet-valid-result.csv")
        ...
        test_results_df['Sweet-ypred'] = y_pred_test  # 保存预测值（非1/0）
        ...
        test_results_csv_filepath = os.path.join(checkpoint_dir, "Sweet-test-result.csv")
    ...
    def create_plot_dataset(df, num_samples=1):
        ...
        y_list = df.loc[sample_indices, 'Has_Sweet'].values
    ...
    def create_plot_dataset(df, indices):
        ...
        y_list = df.loc[indices, 'Has_Sweet'].values
        x_plot = graphs_from_smiles(smiles_list)
        num_atoms_list = []
        # 计算每个样本的原子数量
        for smiles in smiles_list:
